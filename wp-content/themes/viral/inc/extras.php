...
if( !function_exists( 'viral_filter_archive_title' ) ){
	function viral_filter_archive_title($title){
	    if ( is_category() ) {
	        $title = sprintf( __( '%s', 'viral' ), single_cat_title( '', false ) );
	    }
	    return $title;
	}
}
...
if(!function_exists('viral_font_awesome_icon_array')){
	function viral_font_awesome_icon_array(){
...
if( function_exists('viral_check_social_icon_exists')){
	function viral_check_social_icon_exists(){
		...
		if( $facebook || $twitter || $google_plus || $pinterest || $youtube || $linkedin || $instagram ){
			return true;
		}else{
			return false;
		}
...
if(!function_exists('viral_header_text')){
	function viral_header_text(){
		...
		if($viral_left_header_text){
			echo '<span>';
			echo '<i class="fa fa-bookmark" aria-hidden="true"></i>'. esc_html($viral_left_header_text);
			echo '</span>';
		}
	}
}
